...
import io.github.galaxyvn.ggarden.command.framework.CommandMessages;
import io.github.galaxyvn.ggarden.config.CommentedFileConfiguration;
import io.github.galaxyvn.ggarden.hook.PlaceholderAPIHook;
import io.github.galaxyvn.ggarden.locale.Locale;
...
public abstract class AbstractLocaleManager extends Manager {
    ...
    public AbstractLocaleManager(GalaxyPlugin galaxyPlugin) {
        super(galaxyPlugin);
    }
    ...
    private void registerLocale(Locale locale) {
        File file = new File(this.galaxyPlugin.getDataFolder() + "/locale", locale.getLocaleName() + ".lang");
    ...
    public final void reload() {
        File localeDirectory = new File(this.galaxyPlugin.getDataFolder(), "locale");
        ...
        String locale;
        if (this.galaxyPlugin.hasConfigurationManager()) {
            locale = this.galaxyPlugin.getManager(AbstractConfigurationManager.class).getSettings().get("locale").getString();
        } else {
            locale = "en_US";
        }

        File targetLocaleFile = new File(this.galaxyPlugin.getDataFolder() + "/locale", locale + ".lang");
        if (!targetLocaleFile.exists()) {
            targetLocaleFile = new File(this.galaxyPlugin.getDataFolder() + "/locale", "en_US.lang");
            this.galaxyPlugin.getLogger().severe("File " + targetLocaleFile.getName() + " does not exist. Defaulting to en_US.lang");
        }
